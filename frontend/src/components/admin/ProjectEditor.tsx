...
import { uploadImageGetUrl, uploadMedia } from '@/lib/media'
...
export default function ProjectEditor({ projectId }: { projectId?: string }) {
  ...
  const scheduleAutoSave = useCallback(() => {
    ...
    autoSaveTimerRef.current = setTimeout(async () => {
      // Chỉ auto-save khi có title
      if (!formData.title.trim()) return
      // Mutex: skip neu dang save (manual hoac auto-save truoc do)
  ...
  const handleCoverUpload = useCallback(async (file: File) => {
    try {
      ...
      const base64 = await uploadImageGetUrl(file)
        ...
        <div className="flex shrink-0 items-center justify-between border-b border-on-surface/8 bg-surface px-4 py-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <button type="button" onClick={() => setShowPreview(true)}
              className="flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-body-sm text-on-surface-variant hover:bg-on-surface/5">
