...
export async function generateStaticParams() {
  try {
    const res = await fetch(
      `${getServerApiUrl()}/projects?status=published&limit=50&sort=view_count:desc`,
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const project = await serverFetch(`/projects/${slug}`, { tags: ['projects', `project-${slug}`] })
  return buildDetailMetadata(project, SEO_CONFIG)
}

export default async function ProjectDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params
  const project = await serverFetch<any>(`/projects/${slug}`, { tags: ['projects', `project-${slug}`] })
  if (!project) notFound()

  const relatedProjects = project.category?.id
    ? await serverFetchList<any>(`/projects?category_id=${project.category.id}&limit=3&status=published`, {
        tags: ['projects'],
  ...
  return (
    <>
      ...
      <section className="bg-surface py-8 md:py-16">
        <PageContainer>
          <Breadcrumb items={[{ label: 'Dự án', href: '/projects' }, { label: project.title }]} className="mb-6" />

          <Link href="/projects" className="mb-8 inline-flex items-center gap-2 text-body-md text-on-surface-variant transition-colors hover:text-primary">
          ...
          <div className="mb-8">
            {project.category && (
              <p className="font-label text-label-lg uppercase tracking-label-wide text-primary-container">{project.category.name}</p>
            )}
            <h1 className="mt-2 font-headline text-headline-lg text-on-surface md:text-display-md lg:text-display-lg">{project.title}</h1>
            {project.description && (
              <p className="mt-4 max-w-3xl text-body-lg text-on-surface-variant">{project.description}</p>
            )}
          </div>

          {/* Project Info */}
          <div className="mb-10 flex flex-wrap gap-4 md:gap-8 rounded-xl bg-surface-container-low p-6">
            {project.style && (
              <div>
                <p className="font-label text-label-md uppercase tracking-label-wide text-on-surface-variant">Phong cách</p>
                <p className="mt-1 text-body-md text-on-surface">{project.style}</p>
              </div>
            )}
            {project.area && (
              <div>
                <p className="font-label text-label-md uppercase tracking-label-wide text-on-surface-variant">Diện tích</p>
                <p className="mt-1 text-body-md text-on-surface">{project.area}</p>
              </div>
            )}
            {project.location && (
              <div>
                <p className="font-label text-label-md uppercase tracking-label-wide text-on-surface-variant">Địa điểm</p>
                <p className="mt-1 text-body-md text-on-surface">{project.location}</p>
              </div>
            )}
            {project.duration && (
              <div>
                <p className="font-label text-label-md uppercase tracking-label-wide text-on-surface-variant">Thời gian</p>
                <p className="mt-1 text-body-md text-on-surface">{project.duration}</p>
              </div>
            )}
            {project.year_completed && (
              <div>
                <p className="font-label text-label-md uppercase tracking-label-wide text-on-surface-variant">Năm hoàn thành</p>
            ...
            {Array.isArray(project.materials) && project.materials.length > 0 && (
              <div>
                <p className="font-label text-label-md uppercase tracking-label-wide text-on-surface-variant">Vật liệu</p>
                <p className="mt-1 text-body-md text-on-surface">{project.materials.join(', ')}</p>
              </div>
            )}
          </div>
          ...
          {project.gallery && project.gallery.length > 0 && (
            <div className="mt-16">
              <h2 className="mb-8 font-headline text-headline-md text-on-surface">Thư viện hình ảnh</h2>
          ...
          {relatedProjects.length > 0 && (
            <div className="mt-20">
              <h2 className="mb-8 text-center font-headline text-headline-md text-on-surface">Dự án liên quan</h2>
              <div className="card-grid">
                {relatedProjects.map((rp: any) => (
                  <Link key={rp.id} href={`/projects/${rp.slug}`} className="group overflow-hidden rounded-xl bg-surface shadow-ambient-sm transition-shadow hover:shadow-ambient-lg">
                    ...
                    <div className="p-5">
                      {rp.category && <p className="font-label text-label-md uppercase tracking-label-wide text-primary-container">{rp.category.name}</p>}
                      <h3 className="mt-1 font-headline text-title-lg text-on-surface group-hover:text-primary">{rp.title}</h3>
                      {rp.description && <p className="mt-2 line-clamp-2 text-body-sm text-on-surface-variant">{rp.description}</p>}
                    </div>
                  </Link>
                ))}
              </div>
