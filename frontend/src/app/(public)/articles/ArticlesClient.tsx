...
interface ArticlesClientProps {
  initialData?: PaginatedInitialData<Article>
}
...
export default function ArticlesClient({ initialData }: ArticlesClientProps = {}) {
  return (
    <Suspense fallback={<div className="flex min-h-[60vh] items-center justify-center"><div className="h-8 w-8 animate-spin rounded-full border-2 border-primary border-t-transparent" /></div>}>
      <ArticlesContent initialData={initialData} />
    </Suspense>
  )
}

function ArticlesContent({ initialData }: ArticlesClientProps) {
  const { items, meta, page, loading, error, goToPage, refresh, isEmpty } =
    usePaginatedList<Article>({ endpoint: '/articles', limit: 9, initialData })
