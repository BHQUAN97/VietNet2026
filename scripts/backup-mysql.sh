...
DB_CONTAINER="${DB_CONTAINER:-vietnet2026-mysql-1}"
DB_NAME="${DB_NAME:-vietnet}"
DB_USER="${DB_USER:-vietnet}"
LOG_FILE="${LOG_FILE:-/var/log/vietnet/backup.log}"
...
if [ -z "${MYSQL_BACKUP_PASSWORD:-}" ]; then
  if [ -f /opt/vietnet/.env ]; then
    DB_PASSWORD=$(grep '^LQD_DB_PASSWORD=' /opt/vietnet/.env | cut -d= -f2- | tr -d '"' | tr -d "'")
  fi
...
if command -v rclone &> /dev/null; then
  REMOTE="${RCLONE_REMOTE:-r2:vietnet-backups/mysql/}"
