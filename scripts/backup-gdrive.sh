...
# Cron:  0 3 * * * /opt/vietnet/scripts/backup-gdrive.sh
# ──────────────────────────────────────────
...
# Fallback /opt/vietnet/.env (production layout)
if [ -z "${GDRIVE_ENABLED:-}" ] && [ -f /opt/vietnet/.env ]; then
  set -a
  # shellcheck disable=SC1091
  . /opt/vietnet/.env
  set +a
...
# Determine log file — preferred /var/log/vietnet, fallback ./logs/
LOG_DIR="/var/log/vietnet"
